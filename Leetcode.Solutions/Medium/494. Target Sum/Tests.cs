...
public class Tests
{
    ...
    [Timeout(100)]
    public int FindTargetSumWays_SumExists_ReturnsNumOfWaysToSum(int[] nums, int target)
    {
        return new Solution().FindTargetSumWays(nums, target);
    }

    [TestCase(new[] { 1 }, 2, ExpectedResult = 0)]
    public int FindTargetSumWays_SumDoesntExists_Returns0(int[] nums, int target)
    {
        return new Solution().FindTargetSumWays(nums, target);
    }
}
